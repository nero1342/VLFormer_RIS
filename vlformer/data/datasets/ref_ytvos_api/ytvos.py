...
class YTVOS:
    ...
    def createIndex(self):
        ...
        anns, cats, vids, exps = {}, {}, {}, {}
        vidToAnns,catToVids,annToExps = defaultdict(list),defaultdict(list),defaultdict(list)
        ...
        if 'expressions' in self.dataset:
            for exp in self.dataset['expressions']:
                exps[exp['id']] = exp 
    ...
    def loadExps(self, ids=[]):
        """
        Load exps with the specified ids.
        :param ids (int array)       : integer ids specifying vid
        :return: vids (object array) : loaded vid objects
        """
        if _isArrayLike(ids):
            return [self.exps[id] for id in ids]
        elif type(ids) == int:
            return [self.exps[ids]]

    def loadAnnsFromExps(self, ids=[]):
        exps = self.loadExps(ids)
        anno_ids = [exp["anno_id"] for exp in exps]
        annos = self.loadAnns(anno_ids)
        return [self.annsToMask(anno) for anno in annos]
    ...
    def annToMask(self, ann, frameId):
        ...
        if rle is not None:
            ...
            m = np.zeros((h, w))
        # print(t, frameId, m.shape)
        return m

    def annsToMask(self, ann):
        frameLength = len(ann['segmentations']) 
        m = [self.annToMask(ann, i) for i in range(frameLength)]  
        m = np.stack(m, axis = 0)
        # m = maskUtils.decode(ann)
        return m