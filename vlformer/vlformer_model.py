...
from transformers import CLIPTokenizer, CLIPTextModel, BertTokenizer, BertModel, RobertaModel, RobertaTokenizerFast
...
from .modeling.attention import MLP
...
class VLFormer(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.resizer = MLP(512, hidden_dim, hidden_dim, 1)
    ...
    def forward_text(self, captions):
        ...
        return text_word_features, text_word_masks, text_pos
    def forward(self, batched_inputs):
        ...
        captions = self.forward_text(captions)

        outputs = self.sem_seg_head(features, captions)
        ...
        if self.training:
            ...
            return retry_if_cuda_oom(self.inference)(mask_cls_result, mask_pred_result, image_size, height, width)
    ...
    def prepare_targets(self, targets, images):
        ...
        for targets_per_video in targets:
            ...
            # print(gt_instances[-1])
            # print(gt_masks_per_video.sum())
            gt_masks_per_video = gt_masks_per_video[valid_idx].float()          # N, num_frames, H, W
            gt_instances[-1].update({"masks": gt_masks_per_video})

        return gt_instances

    def inference(self, pred_cls, pred_masks, img_size, output_height, output_width):
        # 
        # f = open("object_query_distribution.txt","a")
        if len(pred_cls) > 0:
            ...
            # print(topk_indices.detach().cpu().numpy()[0], file = f)
